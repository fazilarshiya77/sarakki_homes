...
export function PropertyCard({ property }: { property: Property }) {
  return (
    <motion.article
      ...
      className="group relative overflow-hidden rounded-lg border border-border bg-card shadow-soft transition-all duration-500 hover:-translate-y-1.5 hover:shadow-soft-lg"
    >
      <div className="relative h-64 overflow-hidden">
        <div
          className="relative h-full w-full transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.06]"
        >
          {property.image ? (
            <Image
              src={property.image}
        ...
        <button
          ...
          className="absolute right-4 top-4 z-20 flex h-9 w-9 items-center justify-center rounded-full bg-background/90 text-foreground transition-colors duration-300 hover:text-accent-gold-dark"
        >
          <Heart size={16} />
        </button>
      </div>
